...
def predict(state, kernel, x_i):
    ...
    """
    val = 0.0
    # each element of state should be a tuple (coe, x)
    for coe, x in state:
        val += coe * kernel(x, x_i)
...
def update_state(state, kernel, learning_rate, x_i, y_i):
    ...
    coe = learning_rate*(y_i - y_pred)
    state.append((coe, x_i))
...
def train_perceptron(train_path, test_path, kernel_name, kernel, learning_rate):
    ...
    plt.title(f"Kernel: {kernel_name} || Test data, color corresponds to real label")
    plt.legend()
    plt.xlabel("x1")
    plt.ylabel("x2")

    y_pred = np.array([predict(state, kernel, test_x[i, :]) for i in range(test_y.shape[0])])
    plt.figure()
    util.plot_points(test_x, y_pred)
    plt.title(f"Kernel: {kernel_name} || Test data, color corresponds to predicted label")
    plt.legend()
    plt.xlabel("x1")
    plt.ylabel("x2")
    plt.show()
...
def main(train_path, test_path):
    train_perceptron(train_path, test_path, 'dot', dot_kernel, 0.5)
    train_perceptron(train_path, test_path, 'rbf', rbf_kernel, 0.5)


if __name__ == "__main__":
    main('../data/ds5_train.csv', '../data/ds5_test.csv')